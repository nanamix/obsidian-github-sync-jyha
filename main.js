...
// commit-profiles/jyha-korean.js
var require_jyha_korean = __commonJS({
  "commit-profiles/jyha-korean.js"(exports, module2) {
    var profile = {
      name: "jyha-korean",
      types: {
        ADD: { emoji: "\u{1F4DD}", summary: "\uB178\uD2B8 \uCD94\uAC00" },
        UPDATE: { emoji: "\u{1F504}", summary: "vault \uBCC0\uACBD", mixedSummary: "vault \uB3D9\uAE30\uD654" },
        REMOVE: { emoji: "\u{1F525}", summary: "\uB178\uD2B8 \uC0AD\uC81C" },
        RENAME: { emoji: "\u{1F69A}", summary: "\uB178\uD2B8 \uC774\uB984\uBCC0\uACBD" },
        DOCS: { emoji: "\u{1F4DA}", summary: "\uBB38\uC11C \uC218\uC815" }
      },
      bodyLabels: {
        added: "\uCD94\uAC00",
        modified: "\uC218\uC815",
...
var require_commit_message = __commonJS({
  "commit-message.js"(exports, module2) {
    var jyhaKoreanProfile = require_jyha_korean();
    var PROFILES = {
      [jyhaKoreanProfile.name]: jyhaKoreanProfile
    };
    var DEFAULT_PROFILE = "jyha-korean";
    var DOC_EXTENSIONS = /* @__PURE__ */ new Set([".md", ".markdown", ".canvas"]);
    ...
    function resolveProfile(options = {}) {
      const profileName = (options == null ? void 0 : options.profile) || DEFAULT_PROFILE;
    ...
    function selectType(status) {
      ...
      if (total === counts.added && counts.added > 0)
        return "ADD";
      if (total === counts.deleted && counts.deleted > 0)
        return "REMOVE";
      if (total === counts.renamed && counts.renamed > 0)
        return "RENAME";
      if (total === counts.modified && counts.modified > 0 && changedPaths.length > 0 && changedPaths.every(isDocPath)) {
        return "DOCS";
      }
      return "UPDATE";
    }
    function buildChangeLines(status, profile) {
      ...
      if (counts.added > 0)
        lines.push(`- ${labels.added}: ${counts.added}\uAC1C`);
      if (counts.modified > 0)
        lines.push(`- ${labels.modified}: ${counts.modified}\uAC1C`);
      if (counts.deleted > 0)
        lines.push(`- ${labels.deleted}: ${counts.deleted}\uAC1C`);
      if (counts.renamed > 0)
        lines.push(`- ${labels.renamed}: ${counts.renamed}\uAC1C`);
    ...
    function buildMetadataLines(metadata = {}, profile) {
      ...
      if (metadata.vaultName)
        lines.push(`- ${labels.vaultName}: ${metadata.vaultName}`);
      if (metadata.publicIp)
        lines.push(`- ${labels.publicIp}: ${metadata.publicIp}`);
...
${lines.join("\n")}` : "";
    ...
    function buildCommitMessage2(status, metadata = {}, options = {}) {
      ...
      const summary = type === "UPDATE" && hasMultipleChangeTypes && label.mixedSummary ? label.mixedSummary : label.summary;
      return `[${type}] ${label.emoji} ${summary}${buildBody(status, metadata, profile)}`;
    }
    module2.exports = {
      buildCommitMessage: buildCommitMessage2,
      selectType,
      resolveProfile
    };
  }
...
// main.ts
var { buildCommitMessage } = require_commit_message();
...
async function getSyncMetadata(vaultPath) {
